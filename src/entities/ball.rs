...
impl Ball {
    pub fn new(texture: Texture, position: Vec2<f32>, velocity: Vec2<f32>) -> Ball {
        Ball { entity: Entity { position, texture }, velocity }
    }
}
...
impl Updatable for Ball {
    fn update(&mut self, _ctx: &mut Context) {
        self.velocity /= (self.velocity.x * self.velocity.x + self.velocity.y * self.velocity.y).sqrt();
        self.entity.position += self.velocity * BALL_SPEED;

        if self.entity.position.y <= 0.0 || self.entity.position.y + self.entity.dimensions().1 >= WINDOW_HEIGHT as f32 {
