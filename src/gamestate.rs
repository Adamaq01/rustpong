use tetra::{State, graphics, Context, Result, window};
use tetra::math::Vec2;
use tetra::graphics::{Color, DrawParams, Texture};
use crate::entities::{player::Player, ball::Ball};
use tetra::input::Key;
...
impl State for GameState {
    fn update(&mut self, ctx: &mut Context) -> tetra::Result {
        ...
        if let Some(player) = player_hit {
            ...
            self.ball.velocity.y += (player.entity.center().y - self.ball.entity.center().y) / player.entity.dimensions().1;
        }
    ...
    fn draw(&mut self, ctx: &mut Context) -> Result {
        graphics::clear(ctx, Color::rgb(0.137, 0.769, 0.557));

        graphics::draw(ctx, &self.first_player, DrawParams::default());
        graphics::draw(ctx, &self.second_player, DrawParams::default());
        graphics::draw(ctx, &self.ball, DrawParams::default());
...
impl GameState {
    pub fn new(ctx: &mut Context, resources_path: &str) -> tetra::Result<GameState> {
        let first_player_texture = Texture::new(ctx, resources_path.to_string() + "first_player.png")?;
        let first_player_position = Vec2::new(
            16.0,
            (WINDOW_HEIGHT - first_player_texture.height()) as f32 / 2.0,
        );

        let second_player_texture = Texture::new(ctx, resources_path.to_string() + "second_player.png")?;
        ...
        Ok(GameState {
            ...
            second_player: Player::new(second_player_texture, second_player_position, Key::Up, Key::Down),
